    ...
    try
    {
        ...
        while (!itUserContext.value()->userDataStream->atEnd())
        {
            ...
            if (cmd == TaReplayServerSubscribe::ID)
            {
                ...
                qInfo() << "[TaReplayServer::onReadyRead][SUBSCRIBE] gameid,position" << msg.gameId << msg.position;
            ...
            qInfo() << "[TaReplayServer::onReadyRead] end loop";
        }
        qInfo() << "[TaReplayServer::onReadyRead] end function";
    }
...
{
    qInfo() << "[TaReplayServer::sendData] status,data=" << int(status) << data.size();
...
void TaReplayServer::serviceUser(UserContext& user)
{
    ...
    for(;;)
    {
        ...
        qInfo() << "gameid,position,gcount,escrow:" << user.gameId << user.demoFile->tellg() << user.demoFile->gcount() << dataEscrowThreshold;
